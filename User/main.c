...
PC-MCU					Data						UART_channel									Frame
____________________________________________________________________________________					
								button							0		
								X1_Analog						1							[0xFF][0xF(channel)][Data][0xFE]
								fuzzy								2
								Y1_Analog						3
								Y2_Analog						4
				//				LT_Analog						5
...
MCU-PC					Data						UART_channel									Frame
______________________________________________________________________________________
								theta_r							0		
								theta_dot_r					1
								z_offset						2							[0xFF][0xE(channel)][Data][0xFE]
								y_offset						3													
******************************************************************************************************
I2C 
...
//UART_DMA section
	...
	//RX
	#define rxsize 98 //min = 7 channel * 7 bytes = 49
...
float
			trot_A_LHL_y = 1,trot_offset_LHL_y = 2,
			trot_A_LHL_z = 2,trot_offset_LHL_z = 13;

float 
			trot_A_RHL_y = 1,trot_offset_RHL_y = 2,
...
//bound elipse
//float 
//			bound_A_LFL_y = 2,bound_offset_LFL_y = 2,
//			bound_A_LFL_z = 2,bound_offset_LFL_z = 13;

//float 
//			bound_A_RFL_y = 2,bound_offset_RFL_y = 2,
//			bound_A_RFL_z = 2,bound_offset_RFL_z = 13;
//		
//float
//			bound_A_LHL_y = 2,bound_offset_LHL_y = 2,
//			bound_A_LHL_z = 2,bound_offset_LHL_z = 13;

//float 
//			bound_A_RHL_y = 2,bound_offset_RHL_y = 2,
//			bound_A_RHL_z = 2,bound_offset_RHL_z = 13;

//int8_t bound_z_LF,bound_y_LF,
//			 bound_z_RF,bound_y_RF,
//			 bound_z_LH,bound_y_LH,
//			 bound_z_RH,bound_y_RH;
			...
			for (i=0;i<=rxsize-6;i++)
			{
				if ((rxbuffer[i] == 0xFF) && (rxbuffer[i + 6] == 0xFE))
				{
					...
					switch (rxbuffer[i+1])
					{
						...
						case 0xE2:fuzzy_cb = (int8_t)rx_data;break;
...
//		while (Y1_Analog > 10)
//		{
//			bound_y_LF =  	bound_offset_LFL_y + bound_A_LFL_y*cos (2*pi*f*t/1000);
//			bound_z_LF =  	bound_offset_LFL_z + bound_A_LFL_z*sin (2*pi*f*t/1000);
//			LFL_IK_2DOF (bound_y_LF,bound_z_LF);
//			
//			bound_y_RF =   bound_offset_RFL_y + bound_A_RFL_y*cos (2*pi*f*t/1000);
//			bound_z_RF =   bound_offset_RFL_z + bound_A_RFL_z*sin (2*pi*f*t/1000);
//			RFL_IK_2DOF (bound_y_RF,bound_z_RF);
//			
//			bound_y_RH =   bound_offset_RHL_y - bound_A_RHL_y*cos (2*pi*f*t/1000 + pi);
//			bound_z_RH =   bound_offset_RHL_z + bound_A_RHL_z*sin (2*pi*f*t/1000 + pi);
//			RHL_IK_2DOF (bound_y_RH,bound_z_RH);
//					
//			bound_y_LH =   bound_offset_LHL_y - bound_A_LHL_y*cos (2*pi*f*t/1000 + pi);
//			bound_z_LH =   bound_offset_LHL_z + bound_A_LHL_z*sin (2*pi*f*t/1000 + pi);
//			LHL_IK_2DOF (bound_y_LH,bound_z_LH);
//			Delay (10);
//		}
}
			void creep (void)
...
void fuzzy (uint16_t time)
{
	Fuzzy_Fuzzification(theta_r,theta_r_dot);
	Fuzzy_MuCalc(theta_r,theta_r_dot);
	Fuzzy_RuleCheck();
	
	fuzzy_out_z_offset=Fuzzy_Defuzzification_Z();
	fuzzy_out_y_offset=Fuzzy_Defuzzification_Y();
	
	if (fuzzy_cb == 1)
	{
		trot_offset_LFL_z = 13 - fuzzy_out_z_offset;
		trot_offset_RFL_z = 13 - fuzzy_out_z_offset;
		trot_offset_LFL_y = 1 - fuzzy_out_y_offset;
		trot_offset_RFL_y = 1 - fuzzy_out_y_offset;
		
		trot_offset_LHL_z = 13 + fuzzy_out_z_offset;
		trot_offset_RHL_z = 13 + fuzzy_out_z_offset;
		trot_offset_LHL_y = 2  + fuzzy_out_y_offset;
		trot_offset_RHL_y = 2  + fuzzy_out_y_offset;
	}
	else
	{
		trot_offset_LFL_z = 13; 
		trot_offset_RFL_z = 13; 
		trot_offset_LFL_y = 1; 
		trot_offset_RFL_y = 1; 
		
		trot_offset_LHL_z = 13; 
		trot_offset_RHL_z = 13;
		trot_offset_LHL_y = 2; 
		trot_offset_RHL_y = 2;
	}
	Fuzzy_ClearAll();
...
void UART_TX (uint16_t time)
{
	...
	if ((uint16_t)(ticker+time)==GetTicker()) 
	{
		...
		SENDDATA(theta_r_dot,txbuffer, 1);
		SENDDATA(fuzzy_out_z_offset,txbuffer, 2);
		SENDDATA(fuzzy_out_y_offset,txbuffer, 3);
	}
}
void X360_X1_Analog (uint16_t time)
{ 
			trot_A_LFL_y = 2 + X1_Analog/5;
			trot_A_LHL_y = 1 + X1_Analog/5;
	
			trot_A_RFL_y = 2 - X1_Analog/5;
			trot_A_RHL_y = 1 - X1_Analog/5;
}
						/************************ Inverse Kinematics ****************************/
void LFL_IK (float x,float y,float z)
{
